...
function CreateIconPassword(elementAppend,input,divAppendIt){
    ...
    console.log(div);
    console.log(document.querySelector(`.${divAppendIt} .${elementAppend}`));
...
// when user click outside input element rmove icon show password
function removeIconpassword(inputs,event){
    inputs.forEach(ipt => {
        if(ipt!==event && document.querySelector('.show-hide-psrd')){  
            document.querySelector('.show-hide-psrd').remove();
        }
    });
}

export{CreateIconPassword,removeIconpassword};