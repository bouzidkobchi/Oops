...
# async def email_found(email) :
#     return User.query.filter_by(email = email)

# async def card_number_found(card_number) :
#     return User.query.filter_by(card_number = card_number)

# async def user_name_found(user_name) :
#     return User.query.filter_by(user_name = user_name)

# async def add_user(user_name , card_number , email , password , picture = None) :
...
async def change_user_type(modifier:User , target_user:User , new_type:str) :
...
async def block_user(modifier:User , target_user:User) :
...
async def disblock_user(modifier:User , target_user:User) :
...
async def change_favorite_language(target_user:User , new_lang:str) :
...
async def change_password(target_user:User , new_password:str , old_password = None) :
...
async def change_profile_picture(target_user:User , new_picture_path:str) :
...
async def change_email(target_user:User , new_email:str) :
    target_user.email = new_email
    db.session.commit()

async def change_family_name(target_user : User , new_family_name:str) :
    target_user.family_name = new_family_name
    db.session.commit()

async def change_fisrt_name(target_user : User , new_first_name:str) :
    target_user.first_name = new_first_name
    db.session.commit()

async def change_username(target_user:User , new_user_name:str) :
...
async def edit_post(post_object:Post , content:str = None , is_public:bool = None , situation:str = None) :
    change = False
    if content :
...
async def like_post(post_object:Post , liker:User) :
...
async def unlike_post(post_object:Post , liker:User) :
    if post_object in liker.liked_posts :
        liker.liked_posts.remove(post_object)
    db.session.commit()

async def dislike_post(post_object:Post , liker:User) :
...
async def undislike_post(post_object:Post , liker:User) :
    if post_object in liker.liked_posts :
        liker.disliked_posts.remove(post_object)
        db.session.commit()

async def hide_post(post_object:Post , target_user:User) :
...
async def like_comment(comment_object:Comment , liker:User) :
...
async def unlike_comment(comment_object:Comment , liker:User) :
    if comment_object in liker.liked_comments :
        liker.liked_comments.remove(comment_object)
        db.session.commit()

async def dislike_comment(comment_object:Comment , disliker:User) :
...
async def undislike_comment(comment_object:Comment , disliker:User) :
    if comment_object in disliker.disliked_comments :
        disliker.disliked_comments.remove(comment_object)
        db.session.commit()

async def delete_comment(comment_object:Comment , modifier:User) :
