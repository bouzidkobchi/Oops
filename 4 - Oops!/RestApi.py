...
class user_issues :
    ...
    async def user_profile(user_name) :
...
class comment_issues :
    ...
    @login_required
    async def undislike_comment_api() :
        comment_id = request.json.get('comment_id')
        if comment_id :
